#include "stdafx.h"
#include "YUYVReader.h"
#include <io.h>
#include <emmintrin.h>

YUYVReader::YUYVReader()
{
    m_FrameWidth = 0;
    m_FrameHeight = 0;
    m_FrameSize = 0;
...
bool YUYVReader::OpenFile( const char* filename, int width, int height )
{
    if( m_pFile )
        Close();
...
bool YUYVReader::GetFrameRaw( int idx, uint8_t* pDst )
{
...
bool YUYVReader::GetFrameRawLuma( int idx, uint8_t* pDst )
{
    if( !m_pFile || idx < 0 || idx >= m_FrameCnt )
        return false;
...
bool YUYVReader::GetFrameYUVP420( int idx, YUVFrame* pFrame )
{
    if( !m_pFile || idx < 0 || idx >= m_FrameCnt )
        return false;
    ...
    for( int i = 0; i < m_FrameHeight; i += 2 )
    {
        ...
        {
            yplane[2*j] = m_pLine[4*j];
            yplane[2*j+1] = m_pLine[4*j+2];
            uplane[j] = m_pLine[4*j+1];
            vplane[j] = m_pLine[4*j+3];
        }

        yplane += ypitch;
        fread( m_pLine, 1, m_FrameWidth * 2, m_pFile );
        for( int j = 0; j < m_FrameWidth; j++ )
            yplane[j] = m_pLine[2*j];
...
bool YUYVReader::GetFrameYUVP422( int idx, YUVFrame* pFrame )
{
    if( !m_pFile || idx < 0 || idx >= m_FrameCnt )
        return false;
...
bool YUYVReader::GetFrameYUVP444( int idx, YUVFrame* pFrame )
{
    if( !m_pFile || idx < 0 || idx >= m_FrameCnt )
        return false;
    ...
    for( int i = 0; i < m_FrameHeight; i++ )
    {
        ...
        *(int*)(m_pLine + width2) = *(int*)(m_pLine + width2 - 4);
        ...
        for( int j = 0; j < m_FrameWidth; j += 2 )
        {
            int k = j + j;
            yplane[j] = m_pLine[k];
            yplane[j+1] = m_pLine[k+2];
            uplane[j] = m_pLine[k+1];
            uplane[j+1] = (m_pLine[k+1] + m_pLine[k+5] + 1) >> 1;
            vplane[j] = m_pLine[k+3];
            vplane[j+1] = (m_pLine[k+3] + m_pLine[k+7] + 1) >> 1;
        }
...
bool YUYVReader::GetFrameBGRA( int idx, uint8_t* pGBRA, int dstPitch )
{
    if( !m_pFile || idx < 0 || idx >= m_FrameCnt )
        return false;
    ...
    for( int i = 0; i < m_FrameHeight; i++ )
    {
        ...
        *(int*)(m_pLine + width2) = *(int*)(m_pLine + width2 - 4);
        ...
        {
            int k = j + j;
            int y = 1192 * (m_pLine[k] - 16) + 512; // 512 for round
            int u = m_pLine[k+1] - 128;
            int v = m_pLine[k+3] - 128;
            bgra[0] = y + ku * u;
            ...
            y = 1192 * (m_pLine[k+2] - 16) + 512;
            u = ((m_pLine[k+1] + m_pLine[k+5] + 1) >> 1) - 128;
            v = ((m_pLine[k+3] + m_pLine[k+7] + 1) >> 1) - 128;
            bgra[0] = y + ku * u;
            bgra[1] = y + kvm * v + kum * u;
            bgra[2] = y + kv * v;
